<?php
/** @var ViewModels\Instructor\CourseManageViewModel $model */
?>
    ...
    <div class="card shadow-sm mb-4">
        <div class="card-body">
            <div class="row align-items-center">
                <div class="col-md-2">
                    <img src="<?= !empty($model->course->image) ? '/assets/uploads/courses/' . $model->course->image : '/assets/img/default-course.png' ?>"
                         class="img-fluid rounded" alt="Thumbnail">
                </div>
                <div class="col-md-7">
                    <h4 class="fw-bold mb-1"><?= htmlspecialchars($model->course->title) ?></h4>
                    <span class="badge bg-<?= $model->course->status == 'approved' ? 'success' : 'warning' ?> mb-2">
                        <?= $model->course->status == 'approved' ? 'Đã duyệt' : 'Chờ duyệt' ?>
                    </span>
                    <p class="text-muted small mb-0">
                        <i class="bi bi-tag"></i> <?= $model->course->price > 0 ? number_format($model->course->price) . 'đ' : 'Miễn phí' ?> |
                        <i class="bi bi-clock"></i> <?= $model->course->duration_weeks ?> tuần
                    </p>
                </div>
                <div class="col-md-3 text-end">
                    <a href="/instructor/courses/<?= $model->course->id ?>/edit" class="btn btn-outline-primary btn-sm mb-2 w-100">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <a href="/instructor/courses/<?= $model->course->id ?>/lessons/create" class="btn btn-primary btn-sm">
                ...
                <div class="card-body p-0">
                    <?php if ($model->lessons->isEmpty()): ?>
                        <div class="text-center py-5">
                            ...
                            <a href="/instructor/courses/<?= $model->course->id ?>/lessons/create" class="btn btn-outline-primary btn-sm">Tạo bài đầu tiên</a>
                        </div>
                    <?php else: ?>
                        <div class="list-group list-group-flush">
                            <?php foreach ($model->lessons as $lesson): ?>
